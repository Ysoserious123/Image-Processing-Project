...
def modifiedLevelSet(currentState, original, h, dt, alpha, epsilon):
    prev = np.copy(currentState)

    ux, uy, uxy, uxx, uyy = diff.levelSetPartials(prev, h, h)
    prev[1:-1, 1:-1] = prev[1:-1, 1:-1] + (dt/(h**2))*((((ux**2)*uyy) - (2*ux*uy*uxy) + ((uy**2)*uxx)) / (ux**2 + uy**2 + epsilon))[1:-1, 1:-1] - alpha*(prev[1:-1, 1:-1] - original[1:-1, 1:-1])
