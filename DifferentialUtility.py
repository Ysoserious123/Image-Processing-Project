...
def shockFilterPartials(prev, dx, dy):
    ...
    Returns:
        a tuple of the approximated derivatives used in the 2-D Shock-Filter Equation
    '''
    ...
    for i in range(1, len(prev) - 1):
        for j in range(1, len(prev[i]) - 1):
            ...
            forwardX = (prev[i+1, j] - prev[i, j])/(dx)
            forwardY = (prev[i, j+1] - prev[i, j])/(dy)
            backwardX = (prev[i, j] - prev[i-1, j])/(dx)
            backwardY = (prev[i, j] - prev[i, j-1])/(dy)

            if (forwardX*backwardX <= 0): # if they are different signs
                ux[i, j] = 0
            elif (forwardX>0): # if both are positive
                ux[i, j] = min(forwardX, backwardX) # choose the minimum of the finite difference methods
            else:
                ux[i, j] = max(forwardX, backwardX) # if they are both negative, then we choose the largest of the finite difference methods

            if (forwardY*backwardY <= 0):
                uy[i, j] = 0
            elif (forwardY>0):
                uy[i, j] = min(forwardY, backwardY)
            else:
                uy[i, j] = max(forwardY, backwardY)
